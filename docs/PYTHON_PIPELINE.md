...
Команда `import-documents` принимает один или несколько DOCX. `docx_reader.py` читает
OOXML и сохраняет порядок верхнеуровневых абзацев и таблиц в промежуточной модели. Без
профиля строится структурный граф из документов, разделов, абзацев, списков и таблиц.

JSON-профиль позволяет без изменения Python-кода определить:

1. распознавание роли документа по заголовкам, разделам и колонкам таблиц;
2. отображение колонок таблицы в последовательность типов узлов;
3. правила выделения узлов из разделов и выбор родителя;
4. дедупликацию в пределах строки, родителя, раздела, группы или всего графа;
5. тематические словари и стратегии междокументных связей;
6. формы узлов и порядок агрегации статусов.

Профиль `profiles/technology_leadership.json` обрабатывает исходные технологические карты и
единый план. Совместимая команда `import-technology-maps` является короткой оболочкой над
тем же универсальным движком. Имена файлов, индексы таблиц и позиции абзацев не участвуют в
сопоставлении. Внешние API не используются.
...
7. запись диагностик назначения ролей и идентификатора профиля в аудит.
...
Связи `supports`, `develops` и `intersects_with`, созданные по словарям или лексическому
пересечению, содержат `match_score` и `match_basis`. Они являются гипотезами и требуют
проверки профильным экспертом.
